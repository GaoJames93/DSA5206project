...
from methods.negation_maf_savet import NegationSaveT
from methods.negation_only_loss import NegationOnly
...
from methods.negation_nce_MAF_attn import NegationMAFAttn
from methods.negation_nce_MAF_reg import NegationMAFReg
...
def main(configs, method_name):
    ...
    for task in range(tasks):
        ...
        #     method = Negation(configs, surgvlp_model, preprocess, surgvlp.tokenize)
            ...
            method = Negation(configs, surgvlp_model, preprocess, surgvlp.tokenize)
        elif method_name == "savet":
            method = NegationSaveT(configs, surgvlp_model, preprocess, surgvlp.tokenize)
        elif method_name == "negation_only":
            method = NegationOnly(configs, surgvlp_model, preprocess, surgvlp.tokenize)
        ...
        elif method_name == "negation_maf_attn":
            method = NegationMAFAttn(configs, surgvlp_model, preprocess, surgvlp.tokenize)
        elif method_name == "negation_maf_reg":
            method = NegationMAFReg(configs, surgvlp_model, preprocess, surgvlp.tokenize)

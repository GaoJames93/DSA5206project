...
class ImageEncoder(nn.Module):
    def __init__(self, num_classes, pretrained, backbone_name, img_norm): # img_norm default to be False
    ...
    def forward(self, x, get_local=False):
        ...
        # print("local feature shape: ", local_ft.shape)
    ...
    def resnet_forward(self, x, extract_features=False):
        ...
        x = self.model.layer3(x)  # (batch_size, 1024, 14, 14)

        x = self.model.layer4(x)  # (batch_size, 2048, 7, 7)
        local_features = x

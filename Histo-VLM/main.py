import os
import random
import argparse
import yaml
import time
import pandas as pd
from tqdm import tqdm

import torch
import torch.nn.functional as F
import torch.nn as nn
import torchvision.transforms as transforms

from datasets import build_dataset
from datasets.utils import build_data_loader
import clip
import open_clip
from methods import __dict__ as all_methods
from utils import *
from datasets.utils import MultilabelDatasetWrapper

# from transformers import AutoTokenizer, AutoModelForCausalLM

def get_arguments():

    parser = argparse.ArgumentParser()
    parser.add_argument(
        '--base_config', default='configs/base.yaml',
        help='setting of Few-shot CLIP')
    parser.add_argument(
        '--dataset_config', default='configs/nct.yaml',
        help='dataset config')
    parser.add_argument('--opts', default=None, nargs=argparse.REMAINDER)
    args = parser.parse_args()

    cfg = load_cfg_from_cfg_file(args.base_config)
    cfg.update(load_cfg_from_cfg_file(args.dataset_config))
    if args.opts is not None:
        cfg = merge_cfg_from_list(cfg, args.opts)
    return cfg



def main():
    torch.cuda.reset_peak_memory_stats()

    # Load config file
    cfg = get_arguments()

    cache_dir = os.path.join('./caches', cfg['dataset'])
    os.makedirs(cache_dir, exist_ok=True)
    cfg['cache_dir'] = cache_dir

    print("\nRunning configs.")
    print(cfg, "\n")
    # print(all_methods)
    # assert(0)

    method = all_methods[cfg['method']](args=cfg)

    # QuiltNet
    hvl_model, preprocess_train, preprocess_val = open_clip.create_model_and_transforms('hf-hub:wisdomik/QuiltNet-B-32')
    hvl_model.cuda().eval()

    # Prepare dataset
    random.seed(1)
    torch.manual_seed(1)

    print("Preparing dataset.")
    dataset = build_dataset(cfg['dataset'], cfg['root_path'], cfg['shots'])
    classnames = dataset.classnames
    print(classnames)
    print(dataset.template)
    test_loader = build_data_loader(data_source=dataset.test, batch_size=100, is_train=False, tfm=preprocess_val, shuffle=False)

    # Textual features
    print("Getting textual features as Quilt's classifier.")
    print(hvl_model)

    clip_weights = clip_classifier(
        dataset.classnames, dataset.template, hvl_model, dataset = cfg['dataset'])
    print(clip_weights.shape)
    # Pre-load test features
    f_test_time = time.time()
    print("\nLoading visual features and labels from test set.")
    test_features, test_labels = pre_load_features(
        cfg, "test", hvl_model, test_loader)

    total_acc = 0
    predictions = []
    exact_accs = []
    metrics_list = []
    for i in range(cfg['tasks']):
        random.seed(i+1)
        torch.manual_seed(i+1)
        print("Start Training Task:{}".format(str(i+1)))
        few_shot_train_data = dataset.generate_fewshot_dataset_(cfg['shots'], split="train")
        few_shot_val_data = dataset.generate_fewshot_dataset_(cfg['shots'], split="val") 

        if cfg['finetune']:
            train_loader = build_data_loader(
                data_source=few_shot_train_data, batch_size=cfg["batch_size"], tfm=preprocess_train, is_train=True, shuffle=True)
        else:
            train_loader = build_data_loader(
                data_source=few_shot_train_data, batch_size=cfg["batch_size"], tfm=preprocess_train, is_train=True, shuffle=False)
        val_loader = build_data_loader(
            data_source=few_shot_val_data, batch_size=cfg["batch_size"], tfm=preprocess_val, is_train=False, shuffle=False)

        loss, acc, metrics = method(train_loader=train_loader,
                        val_loader=val_loader,
                        test_loader=test_loader,
                        test_features=test_features,
                        test_labels=test_labels,
                        text_weights=clip_weights,
                        model=hvl_model,
                        classnames=classnames)
        
        print('Final Accuracy on task {}: {}'.format(str(i+1), acc))
        metrics_list.append(metrics)
        predictions.append(acc)

    if cfg["type"] == "multilabel":
        test_stats = compute_multilabel_metric_stats(metrics_list)
        final_metrics = test_stats
    else:
        tasks_acc, tasks_std = compute_confidence_interval(predictions)
        test_stats = {}
        test_stats['acc'] = tasks_acc
        test_stats['std'] = tasks_std

        print('Total Accuracy and std on {} tasks: {:.4f} , {:.4f}'.format(
            str(cfg['tasks']), tasks_acc, tasks_std))
        final_metrics = average_task_results(metrics_list)
    print(final_metrics)
    if not os.path.exists(cfg['output_dir']):
        os.mkdir(cfg['output_dir'])
    csv_path = os.path.join(cfg['output_dir'], cfg['dataset']+".csv")
    write_to_csv(cfg, csv_path, test_stats)
    # Save experiment metrics
    save_results(final_metrics, cfg['output_dir'], id_experiment=cfg['dataset']+"_"+cfg["method"]+"_"+str(cfg["shots"]),
                 id_metrics="metrics")

def write_to_csv(cfg, path, test_stats):
    
    try:
        res = pd.read_csv(path)
    except:
        res = pd.DataFrame()
    records = res.to_dict('records')
    if cfg['method'] == "TIPAdapter" and cfg["finetune"]:
        test_stats['method'] = "TIPAdapter-F"
    else:
        test_stats['method'] = cfg['method']
    if cfg['method'] == "TIPAdapter":
        test_stats['batch_size'] = 256
    else:
        test_stats['batch_size'] = 32
    test_stats['num_shots'] = cfg['shots']
    test_stats['tasks'] = cfg['tasks']
    test_stats['epochs'] = cfg['train_epoch']

    records.append(test_stats)
    # Save back to dataframe
    df = pd.DataFrame.from_records(records)
    df.to_csv(path, index=False)

if __name__ == '__main__':
    main()


...
class ClipAdapter(FSCLIPmethod):
    ...
    def forward(self,
        ...
        clip_ad_model = CustomCLIP(model, dtype)
        ...
        for train_idx in range(self.cfg['train_epoch']):
            ...
            acc = cls_acc(logits, val_labels)
            metrics = classification_metrics(val_labels.cpu().numpy(), logits.detach().cpu().numpy())
            acc = metrics["aca"]
...
class CustomCLIP(nn.Module):
    ...
    def __init__(self, clip_model, dtype):
        ...
        self.dtype = dtype
        self.adapter = Adapter(1024, 2).to(dtype)

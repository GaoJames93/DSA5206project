...
class MultilabelLinearProbe(FSCLIPmethod):
    '''
    Linear Probe method
    '''
    ...
    def __init__(self, args: argparse.Namespace):
        ...
        self.visual_feature_width = 512
        self.hidden_layer_width = 64
        self.num_classes = args['num_classes']
        
        self.classifier = torch.nn.Linear(self.visual_feature_width, self.num_classes, bias=False).to('cuda')
    ...
    def forward(self,
        ...
        def draw_curve_loss(current_epoch):
            ...
            if current_epoch == 0:
                ax0.legend()
            fig_loss.savefig(os.path.join('./','Multilabel_LP_randominit'+'_'+str(self.shot)+'shots_loss.png'))
        ...
        optimizer = torch.optim.Adam(self.classifier.parameters(), lr=self.lr, weight_decay=1e-5)

        for epoch in range(self.epoch):
            optimizer.zero_grad()
            
            # Forward pass (skip last ReLU for classification)
            ...
            draw_curve_loss(epoch)
            print(f'Epoch {epoch+1}/{self.epoch}, Loss: {loss.item():.4f}')
        
        # Test evaluation
        with torch.no_grad():
            # Forward pass for test features
            test_logits = self.classifier(test_features)
            
            # Compute predictions and accuracy
            test_probs = torch.sigmoid(test_logits)
            preds = (test_probs > self.threshold).int()
            
            print(test_labels)
            print(preds)
            metrics = multilabel_metrics(test_labels, preds)
...
class LinearProbe(FSCLIPmethod):
    ...
    def forward(self,
        ...
        def binary_search(c_left, c_right, step, test_acc_step_list):
            ...
            print(pred)
            metrics = classification_metrics(test_labels.cpu().numpy(), pred, "lp")
            print('Metrics: aca=%2.5f - kappa=%2.3f - macro f1=%2.3f' % (metrics["aca"], metrics["kappa"],
                                                                     metrics["f1_avg"]))
            test_acc_step_list[step] = metrics["aca"]
            return np.power(10, c_left), np.power(10, c_right), step, test_acc_step_list, metrics

        for step in range(self.num_step):
            print('step is ',step)
            c_left, c_right, step, test_acc_step_list, metrics = binary_search(c_left, c_right, step, test_acc_step_list)
        # save results of last step
        end = time.time()
        total_time = end-start
        print('total time is ',total_time)
        acc_test_final = test_acc_step_list[-1]
        metrics["aca"] = acc_test_final
        
        return 0, acc_test_final, metrics

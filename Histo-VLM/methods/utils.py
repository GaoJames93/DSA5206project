...
def multilabel_metrics(targets, preds):
    ...
    precisions = tp / (tp + fp + 1e-10)         # [C]
    recalls = tp / (tp + fn + 1e-10)            # [C]
    f1s = 2 * (precisions * recalls) / (precisions + recalls + 1e-10)  # [C]
    
    macro_prec = precisions.mean()
    macro_rec = recalls.mean()
    macro_f1 = f1s.mean()

    return {
        'micro_precision': micro_prec.item(),
        'micro_recall': micro_rec.item(),
        'micro_f1': micro_f1.item(),
        'macro_precision': macro_prec.item(),
        'macro_recall': macro_rec.item(),
        ...
        with torch.no_grad():
            ...
            for augment_idx in range(cfg['augment_epoch']):
                ...
                for i, (images, target) in enumerate(tqdm(train_loader_cache)):
                    ...
                    # image_features : [bc_size, embedding_len (512)]
...
def multilabel_compute_centroids_alpha(z_s: torch.tensor,
                      y_s: torch.tensor):
    """
    inputs:
        z_s : torch.Tensor of size [batch_size, s_shot, d]
        y_s : torch.Tensor of size [batch_size, s_shot, num_class]

    updates :
        centroids : torch.Tensor of size [n_task, num_class, d]
    """
    centroids = (y_s*z_s/ y_s.sum(-2, keepdim=True)).sum(1)  # [batch, K, d]
    return centroids
...
def multilabel_compute_centroids(z_s: torch.tensor,
                      y_s: torch.tensor):
    """
    inputs:
        z_s : torch.Tensor of size [batch_size, s_shot, d]

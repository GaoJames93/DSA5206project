...
class TIPAdapter(FSCLIPmethod):
    ...
    def forward(self,
        ...
        # print(cache_keys.shape) [512, num_shot*num_class]
        ...
        if not self.finetune:
            ...
            tip_logits = 100. * test_features @ text_weights
            metrics = classification_metrics(test_labels.cpu().numpy(), tip_logits.detach().cpu().numpy())
            print("\n**** Zero-shot CLIP's val accuracy: {:.2f}. ****\n".format(acc))
            print("\n**** Zero-shot CLIP's test accuracy: {:.4f}. ****\n".format(metrics["aca"]))
            print(2/0)
            # Tip-Adapter
            
            affinity = val_features @ cache_keys
            cache_logits = ((-1) * (beta - beta * affinity)).exp() @ cache_values.to(affinity.dtype)
            ...
            cache_logits = ((-1) * (best_beta - best_beta * affinity)).exp() @ cache_values.to(affinity.dtype)
        ...
        adapter = nn.Linear(cache_keys.shape[0], cache_keys.shape[1], bias=False).cuda()
        adapter.weight = nn.Parameter(cache_keys.t())
        print(adapter.weight.dtype)
        ...
        for train_idx in range(self.cfg['train_epoch']):
            ...
            tip_logits = clip_logits + cache_logits * alpha
            acc = cls_acc(tip_logits, val_labels)
            metrics = classification_metrics(val_labels.cpu().numpy(), tip_logits.detach().cpu().numpy())
            acc = metrics["aca"]
